...
public final class GuiUtils {
    ...
    public static void drawTexturedModalRect(int x, int y, int u, int v, int width, int height, float zLevel) {
        ...
        builder.addVertex(x, y + height, zLevel).setUv(u * uScale, (v + height) * vScale);
        builder.addVertex(x + width, y + height, zLevel).setUv((u + width) * uScale, (v + height) * vScale);
        builder.addVertex(x + width, y, zLevel).setUv((u + width) * uScale, v * vScale);
        builder.addVertex(x, y, zLevel).setUv(u * uScale, v * vScale);
    ...
    /** Draws the whole texture stretched into the given rectangle (uv 0..1). */
    public static void bindAndDrawFullTexture(ResourceLocation texture, GuiGraphics guiGraphics, int x, int y,
                                              int width, int height) {
        bindAndDrawScaledTexture(texture, guiGraphics, x, y, width, height, width, height, width, height);
    }
}
